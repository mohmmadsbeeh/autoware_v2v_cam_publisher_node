...
LNI::CallbackReturn UdpSenderNode::on_configure(const lc::State & state)
{
  ...
  // Use the topic name parameter instead of hardcoded "udp_write"
  m_subscriber = this->create_subscription<udp_msgs::msg::UdpPacket>(
    m_topic_name, qos, callback);

  RCLCPP_DEBUG(get_logger(), "UDP sender successfully configured.");
  RCLCPP_INFO(get_logger(), "Subscribed to topic: %s", m_topic_name.c_str());
...
void UdpSenderNode::get_params()
{
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The ip parameter provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The port parameter provided was invalid");
    throw ex;
  }

  // Declare and get the topic_name parameter
  try {
    m_topic_name = declare_parameter<std::string>("topic_name", "udp_write");
  } catch (rclcpp::ParameterTypeException & ex) {
    RCLCPP_ERROR(get_logger(), "The topic_name parameter provided was invalid");
    throw ex;
  }

  RCLCPP_INFO(get_logger(), "ip: %s", m_ip.c_str());
  RCLCPP_INFO(get_logger(), "port: %i", m_port);
  RCLCPP_INFO(get_logger(), "topic_name: %s", m_topic_name.c_str());
}
